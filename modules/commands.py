...
async def update_logic(message, client):
    if message.author in prefix["bot_owner"]:
...
async def restart_logic(message, client):
    if message.author == prefix["bot_owner"]:
...
async def broadcast_server(message, client):
    ...
    for server in client.servers:
        try:
            if storage[message.server.id] == "broadcast0" and message.author == prefix["bot_owner"]:
...
async def broadcast_server_toggle(message, client):
    if message.content.split(' ')[1] == 'off':
        if message.author == message.server.owner or message.author == prefix["bot_owner"]:
...
async def about(message, client):
    ...
    a_details.add_field(name='People I can see:', value=len([i for i in client.servers for channel in i.channels]), inline=True)
