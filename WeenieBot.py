...
async def on_member_ban(member):
    if leavemsg[member.server.id] == 1:
        for i in channel.server.channels:
            if i.id == log_channel[str(member.server.id)]:
...
async def on_channel_delete(channel):
    for i in channel.server.channels:
        if i.id == log_channel[str(channel.server.id)]:
...
async def on_message_edit(before, after):
    for i in before.server.channels:
        if i.id == log_channel[str(before.server.id)]:
            r = lambda: random.randint(0,255)
            rr = ('0x%02X%02X%02X' % (r(),r(),r()))
...
async def on_message_delete(message):
    for i in message.server.channels:
        if i.id == log_channel[str(message.server.id)]:
            r = lambda: random.randint(0,255)
            rr = ('0x%02X%02X%02X' % (r(),r(),r()))
